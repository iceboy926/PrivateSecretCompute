...
int aes_encrypt_data(const unsigned char *key, unsigned int keySize,unsigned char *input, size_t inputlen, unsigned char *output, size_t *poutputlen )
{   
    if( inputlen % 16 )
    ...
    aes_setkey_enc(&aesctx, key, keySize);

    size_t j = 0,ret =0;
    while(j < inputlen)
    {
        ret = aes_crypt_ecb(&aesctx, AES_ENCRYPT, input + j,output + j);
        if(ret != 0)
        {
            break;
        }
...
int aes_decrypt_data(const unsigned char *key, unsigned int keySize,unsigned char *input, size_t inputlen, unsigned char *output, size_t *poutputlen )
{   
    if( inputlen % 16 )
        return( POLARSSL_ERR_AES_INVALID_INPUT_LENGTH );
    ...
    aes_setkey_dec(&aesctx, key, keySize);

    size_t j = 0,ret =0;
    while(j < inputlen)
    {
        ret = aes_crypt_ecb(&aesctx, AES_DECRYPT, input + j,output + j);
        if(ret != 0)
        {
            break;
        }
...
int aes_self_test( int verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        else
        {
            ...
            for( j = 0; j < 10000; j++ )
            {
                ...
                nnl_memcpy( tmp, prv, 16 );
                nnl_memcpy( prv, buf, 16 );
                nnl_memcpy( buf, tmp, 16 );
            }
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        nnl_memcpy( iv,  aes_test_cfb128_iv, 16 );
        nnl_memcpy( key, aes_test_cfb128_key[u], 16 + u * 8 );
        ...
        if( v == AES_DECRYPT )
        {
            nnl_memcpy( buf, aes_test_cfb128_ct[u], 64 );
        ...
        {
            nnl_memcpy( buf, aes_test_cfb128_pt, 64 );
            aes_crypt_cfb128( &ctx, v, 64, &offset, iv, buf, buf );
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        nnl_memcpy( nonce_counter, aes_test_ctr_nonce_counter[u], 16 );
        nnl_memcpy( key, aes_test_ctr_key[u], 16 );
        ...
        if( v == AES_DECRYPT )
        {
            ...
            nnl_memcpy( buf, aes_test_ctr_ct[u], len );
        ...
        {
            len = aes_test_ctr_len[u];
            nnl_memcpy( buf, aes_test_ctr_pt[u], len );
